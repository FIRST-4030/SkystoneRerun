...
public class TestAutoOp extends LinearOpMode {
    ...
    public static Pose2dWrapper startPoseBlue = new Pose2dWrapper(-36, 64, Math.PI * 1.5);
    public static SplineConstants PLAT_POINT = new SplineConstants( -58, -28, 1.5707,0);
    ...
    public static Pose2dWrapper bridgePoseBlue = new Pose2dWrapper(25, 45, 0);
    public static double SPLINE_MAX_VEL = 20, SPLINE_MAX_ACCEL = 30;
    public Servo claw;
    public Servo HR;
    public Servo HL;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (inputHandler.buttonA.pressed) red = !red;

        if(red){ drive.setPoseEstimate(startPose.toPose2d()); } else { drive.setPoseEstimate(startPoseBlue.toPose2d()); }

        Trajectory traj1 = drive.trajectoryBuilder(drive.getPoseEstimate())
                .splineTo(new Vector2d(PLAT_POINT.x, (red ? 1 : -1) * PLAT_POINT.y), (red ? 0 : Math.PI) + PLAT_POINT.heading, new MecanumVelocityConstraint(SPLINE_MAX_VEL, DriveConstants.TRACK_WIDTH), new ProfileAccelerationConstraint(SPLINE_MAX_ACCEL))
                .build();
        Trajectory traj2 = drive.trajectoryBuilder(traj1.end())
                .lineToLinearHeading(new Pose2dWrapper(PLAT_POINT_2.x, (red ? 1 : -1) *PLAT_POINT_2.y, (red ? 0 : Math.PI) + PLAT_POINT_2.heading).toPose2d())
                .build();
        Trajectory traj3;
        if(red) {
            ...
            traj3 = drive.trajectoryBuilder(traj2.end())
                    .lineToLinearHeading(bridgePoseBlue.toPose2d())
                    .build();
        }
